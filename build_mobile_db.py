...
def download_and_encode(product_id):
    """Download the 400w card image, re-encode at quality 85, and save to IMAGE_DIR."""
    os.makedirs(IMAGE_DIR, exist_ok=True)
    path = os.path.join(IMAGE_DIR, f"{product_id}.jpg")
    if os.path.exists(path):
        return product_id, True

    url = f"https://tcgplayer-cdn.tcgplayer.com/product/{product_id}_400w.jpg"
    try:
...
def compress_database():
    ...
    print("3. Fetching catalog images to local staging directory...")
    os.makedirs(IMAGE_DIR, exist_ok=True)
    cursor.execute("SELECT product_id FROM cards")
    product_ids = [r[0] for r in cursor.fetchall()]

    print(f"Downloading {len(product_ids)} images at 400w...")

    downloaded_count = 0
    processed_count = 0
    with ThreadPoolExecutor(max_workers=15) as executor:
        for pid, ok in executor.map(download_and_encode, product_ids):
            if ok:
                downloaded_count += 1
            processed_count += 1
            if processed_count % 1000 == 0:
                print(f"Processed {processed_count}/{len(product_ids)} images...")

    print(f"Downloaded {downloaded_count} images to {IMAGE_DIR}/")

    print("4. Compressing catalog images into catalog_images.zip...")
    shutil.make_archive('catalog_images', 'zip', root_dir=IMAGE_DIR)

    print("5. Optimizing and vacuuming database file size...")
    cursor.execute("VACUUM")
    conn.close()
    print("Mobile Catalog Build Complete! The database is fully optimized and images are staged in catalog_images/ and catalog_images.zip.")
