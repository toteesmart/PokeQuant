...
if page == "Search & Buy":
    ...
    if st.session_state.cart:
        ...
        # Robust CSS injection targeting the specific DOM node generated by the st.container below
        st.markdown("""
        <style>
        /* Define consistent theming colors for the drawer */
        :root {
            --drawer-bg: #1e2127; /* Darker than standard bg to pop slightly */
            --drawer-border: rgba(255,255,255,0.1);
        }

        /* ----- MOBILE STYLES: Sticky Right-Side Drawer ----- */
        @media (max-width: 768px) {
            /* 1. Target the specific layout block that contains our hidden span anchor */
            div[data-testid="stVerticalBlock"]:has(> div.element-container span#lot-drawer-target) {
                ...
                /* Hide everything off-screen except for a 48px sliver (the pull tab) */
                transform: translateX(calc(100% - 48px)); 
            }
            
            /* 2. Slide the entire widget into view when the expander is toggled open */
            div[data-testid="stVerticalBlock"]:has(> div.element-container span#lot-drawer-target):has(details[open]) {
                transform: translateX(0);
            }
            
            /* Clean up default Streamlit expander styles */
            ...
            /* Rotate the title text to face inwards toward the screen */
            div[data-testid="stVerticalBlock"]:has(> div.element-container span#lot-drawer-target):not(:has(details[open])) summary p {
                ...
                font-weight: 700;
                font-size: 1.05rem;
                color: #10b981; /* Emphasize the money/deal aspect */
                text-align: center;
                white-space: nowrap;
                letter-spacing: 0.5px;
            }

            /* Hide the default dropdown chevron arrow when closed */
            div[data-testid="stVerticalBlock"]:has(> div.element-container span#lot-drawer-target):not(:has(details[open])) summary svg {
                display: none;
            }
            
            /* 4. The Header Area (Summary when OPEN) */
            ...
            /* 5. The Content Body (Scrollable List) */
