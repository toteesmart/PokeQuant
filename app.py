...
if page == "Search & Buy":
    ...
    # --- SECTION 1: STICKER PRICE RULES ---
    st.subheader("1. Table Sticker Pricing Rules")
    s_col1, s_col2, s_col3 = st.columns(3)
    
    with s_col1:
        s_mode = st.selectbox(
            "Rounding Method",
            ["Custom Cutoff", "Always Ceil ($1)", "Exact Market", "Ending in .99"],
            index=["Custom Cutoff", "Always Ceil ($1)", "Exact Market", "Ending in .99"].index(settings["sticker_rules"]["mode"]) if settings["sticker_rules"]["mode"] in ["Custom Cutoff", "Always Ceil ($1)", "Exact Market", "Ending in .99"] else 0
        )
    with s_col2:
        s_cutoff = st.number_input(
            "Floor/Ceil Cutoff Threshold", 
            min_value=0.05, max_value=0.95, value=float(settings["sticker_rules"]["cutoff_threshold"]), step=0.05,
            help="Decimals at or below this value round down. Above this value round up. (Only applies to Custom Cutoff mode)"
        )
    with s_col3:
        s_min = st.number_input(
            "Minimum Sticker Price ($)", 
            min_value=0.25, value=float(settings["sticker_rules"]["min_sticker_price"]), step=0.25
        )

    st.divider()

    # --- SECTION 2: CONDITION DISCOUNTS ---
    st.subheader("2. Condition Multipliers (% of Near Mint)")
    c_cols = st.columns(4)
    ...
    # --- SECTION 3: CASH OFFER TIERS ---
    st.subheader("3. Cash Offer Scaling Tiers")
    ...
    if st.button("Save Configuration to Cloud", type="primary", use_container_width=True):
        new_settings = {
        ...
        with st.spinner("Pushing configuration to Turso..."):
            save_vendor_settings(new_settings)
            st.session_state["vendor_settings"] = new_settings
        st.success("Configuration updated! All active inventory deltas and future lot calculations will now use these margins.")
        time.sleep(1.5)
        st.rerun()