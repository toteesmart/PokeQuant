...
- **Edge API Gateway:** Cloudflare Worker (`worker.js`). An edge reverse proxy verifying Supabase RS256 JWTs via the project JWKS endpoint (with a legacy `X-Beta-Key` fallback during migration), handling Cross-Origin Resource Sharing (CORS), and forwarding batch pipeline requests to Turso.
