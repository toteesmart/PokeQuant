...
function buildInventoryImageUrl(
  ...
  // URLs generated by the TCGplayer/CDN fallbacks should be overridden by
  // the offline catalog image when available.
  const isGeneratedCdn =
    trimmed.startsWith(INVENTORY_IMAGE_BASE) ||
    trimmed.startsWith(CATALOG_IMAGE_BASE);

  // 1. inventory.custom_image_data (valid URL, local file URI, or base64/ data URI).
  ...
  // 2. Offline catalog image shipped with the downloaded catalog (local file or remote 400w fallback).
  if (catalogImageUrl) {
  ...
  // 4. Build a catalog image URL (local when extracted, otherwise remote 400w fallback).
