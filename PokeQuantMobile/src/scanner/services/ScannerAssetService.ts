...
import {
  ...
  SCANNER_SIDECAR_JP_MANIFEST_URL,
  SCANNER_SIDECAR_JP_BIN_URL,
} from '../../constants/api';
import { catalogJpImagesReady } from '../../services/CatalogImageService';
...
const jpEmbeddingsDir = new Directory(scannerDir, 'catalog_embeddings_jp');
...
type ScannerAsset = {
  ...
  sizeHint: number;
  // Optional assets (the JP embedding sidecar) are skipped when the feature is
  // off and never block the base ready state if a download fails.
  optional?: boolean;
};
...
// Japanese embedding sidecar — downloaded only when the user has opted into
// Japanese support (the JP image pack). EmbeddingCache merges it into the same
// productId → vector map when present.
const SCANNER_JP_ASSETS: ScannerAsset[] = [
  {
    name: 'catalog_embeddings_jp/manifest.json',
    url: SCANNER_SIDECAR_JP_MANIFEST_URL,
    sizeHint: 700_000,
    optional: true,
  },
  {
    name: 'catalog_embeddings_jp/embeddings.bin',
    url: SCANNER_SIDECAR_JP_BIN_URL,
    sizeHint: 150_000_000,
    optional: true,
  },
];
...
function assetFile(name: string): File {
  if (name.startsWith('catalog_embeddings_jp/')) {
    return new File(jpEmbeddingsDir, name.split('/')[1]);
  }
...
export async function ensureScannerAssets(force = false): Promise<void> {
  ...
  // The JP sidecar is wanted only when the user has opted into the Japanese
  // image pack — downloading JP images is the explicit signal of JP intent.
  const wantJp = catalogJpImagesReady();
  const jpMissing =
    wantJp &&
    SCANNER_JP_ASSETS.some((a) => !assetFile(a.name).exists);

  if (!force && (await areScannerAssetsReady()) && !jpMissing) {
  ...
  return trackScannerDownload(async () => {
    ...
    if (wantJp) {
      jpEmbeddingsDir.create({ intermediates: true, idempotent: true });
    }
    ...
    try {
      ...
      const assets = wantJp
        ? [...SCANNER_ASSETS, ...SCANNER_JP_ASSETS]
        : SCANNER_ASSETS;
      const totalSizeHint = assets.reduce((sum, a) => sum + a.sizeHint, 0);

      const bundled = copyBundledAssets();
      let completedBytes = assets
        .filter((a) => bundled.has(a.name) || assetFile(a.name).exists)
        ...
        const fileLabel =
          asset.name === 'card_detector.tflite'
            ...
            : asset.name === 'mobileclip_s2_image_fp16.tflite'
              ...
              : asset.name.startsWith('catalog_embeddings_jp/')
                ? 'Japanese card embeddings'
                : 'card embeddings';
        ...
        try {
          await File.downloadFileAsync(`${asset.url}?v=${Date.now()}`, dest, {
            ...
            onProgress: (data: DownloadProgress) => {
              const pct = Math.min(
                1,
                (base + data.bytesWritten) / totalSizeHint
              );
          ...
          // Optional (JP) assets never fail the bundle — a partial pair is
          // cleaned up and retried on the next pass.
          ...
          logInfo(`Optional scanner asset skipped: ${asset.name}`, err);
          continue;
        }
